...
trait HasCrudPrepareQuery
{
                    ...
                    if (Str::contains($v[1], '.')) {
                        ...
                        $query->whereHas($rel, function ($query) use ($v, $ex, $operations) {
                            ...
                            if ($op == 'IN') {
                                ...
                                $query->whereNotIn($field,  explode('|', $v[3]));
                            } elseif ($op == 'IS NULL') {
    ...
    public function __prepareQuerySortOrderList()
    {
        ...
        if (Str::contains($order_by, '.')) {
            ...
            $currentQuery = app($this->model);
            $currentTable = $currentQuery->getTable();

            for ($i = 0; $i < count($parts) - 1; $i++) {
                $rel = implode('.', array_slice($parts, 0, $i + 1));

                if (!isset($relations[$rel])) {
                    throw new \Exception("Relation {$rel} is not defined.");
                }

                $relConfig = $relations[$rel];
                $joinTable = $relConfig['table'];
                $joinField = $relConfig['field'];

                if (isset($relConfig['is_has']) && $relConfig['is_has']) {
            ...
            if (!$this->dontAddSelectOnPrepareSortAndOrder) {
                $query->addSelect(app($this->model)->getTable() . ".*");
            }

            $query->orderBy("{$currentTable}.{$field}", $sort_by);
        } else {
            if ($this->disableOrderList) {
                return $query;
            }
